...
void check_order_buttons(int *p_g_order_buttons){  //Dette blir feil ettersom den må huske på verdien
    for(int i=0; i<12; i++){                           // Nå vil den bli 0 hvis man slipper den
        if(i==1 || i==9){
            continue;
        }
        int floor=((i/3));        //Heiltalsdivisjon
        int button_type= (i%3);
        if (*(p_g_order_buttons+i)==0){     //prøvde å fikse her

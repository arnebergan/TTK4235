...
void drive_elevator(int *p_g_direction_piority, int *p_g_order_buttons, int *p_g_floor, int *p_g_motor_direction){
    if(*p_g_motor_direction==0){
        if (*p_g_direction_piority){                  //priority upwards
            for(int i=(((*p_g_floor)+1)*3); i<12; i++){
            ...
            for(int i=(((*p_g_floor)+1)*3-4); i<-1; i--){ 
                if (*(p_g_order_buttons+i)){
                    elevio_motorDirection(-1);
                    *p_g_motor_direction=-1;
                    *p_g_direction_piority=-1;
